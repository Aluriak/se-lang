...
from objects import OBJECTS, OrbitInfo, Ring
from objects_builder import ref


def compile_to_se(system_name:str, orbits:dict or tuple, objects:dict, se_addons_dir:str,
    ...
    star_file = os.path.expanduser(os.path.join(se_addons_dir, 'addons/catalogs/stars/', system_name + '.sc'))
    planet_file = os.path.expanduser(os.path.join(se_addons_dir, 'addons/catalogs/planets/', system_name + '.sc'))
    if not overwrite:
    ...
    for parent, child, orbit in orbits:
        if isinstance(child, Ring):
            ...
            for body, angle_step in zip(child.bodies, child.angle_steps):
                ...
                orbit = OrbitInfo(**base_orbit)  # orbit specific to the child
